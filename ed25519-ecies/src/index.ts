...
export class Ed25519Ecies {
  ...
  static sharedSecretFromEd25519Keys = (
  ...
  static ivkEkMFromEd25519Keys = async (
    secretKey: Uint8Array,
    publicKey: Uint8Array
  ): Promise<ivkEkM> => {
  ...
  static encrypt = async (
    ...
    const fromKeypair = from
      ? sign.keyPair.fromSecretKey(from)
      : sign.keyPair();
    const Rbuf = fromKeypair.publicKey;
    const { iv, kE, kM } = await this.ivkEkMFromEd25519Keys(
